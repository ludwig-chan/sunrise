...
interface CharacterState {
  ...
  temperature: number;
  // 记录已警告的属性，避免重复 toast
  warnedStats: Record<string, boolean>;
}
...
export const useCharacterStore = defineStore('character', {
  ...
  actions: {
    ...
    async hourlyUpdate() {
      ...
      // ===== 属性过低 toast 警告（去重：每个阈值只警告一次） =====
      const checkThreshold = (stat: string, value: number, threshold: number, msg: string) => {
        const key = `${stat}_${threshold}`
        if (value < threshold && !this.warnedStats[key]) {
          this.warnedStats[key] = true
          toast({ message: msg, type: 'warning' })
        } else if (value >= threshold + 10) {
          // 恢复超过阈值+10后重置，允许再次警告
          this.warnedStats[key] = false
        }
      }
      checkThreshold('health', this.health, 30, '⚠️ 血量过低，请注意休息！')
      checkThreshold('energy', this.energy, 20, '⚠️ 体力不支，需要休息！')
      checkThreshold('satiety', this.satiety, 20, '⚠️ 饥饿难耐，赶快吃东西！')
      checkThreshold('mood', this.mood, 20, '⚠️ 心情极差，需要放松一下！')
      checkThreshold('hygiene', this.hygiene, 20, '⚠️ 卫生状况糟糕，该洗澡了！')
      checkThreshold('temperature_low', this.temperature, 33, '⚠️ 体温过低，快去取暖！')
      checkThreshold('temperature_high', this.temperature, 39, '⚠️ 体温过高，需要降温！')
    },
