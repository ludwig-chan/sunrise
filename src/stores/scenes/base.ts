...
import { emitter, floatingText } from '../../utils/eventBus';
...
// ===== 农田种子系统 =====
// 农田生长所需时间（毫秒）：约 3 游戏天
// 游戏1天 = 24游戏小时 × 10000毫秒/游戏小时（默认 timeSpeed）= 240000ms
// 3天 × 240000ms/天 = 720000ms（约 12 分钟现实时间）
  ...
  actions: {
    ...
    async explore() {
      ...
      if (eventRoll < 0.3) {
        ...
        floatingText('+' + amount + ' ' + picked.name, 'gain');
    ...
    async build(recipeType: string) {
      ...
      for (const [resourceType, required] of Object.entries(recipe.cost)) {
        ...
        floatingText('-' + required + ' ' + (RESOURCE_NAMES[resourceType] ?? resourceType), 'loss');
      }
    ...
    addCampfireFuel(building: GameBuilding, fuelItemId: string): boolean {
      ...
      floatingText('-1 ' + fuelDef.name, 'loss');
    ...
    startCampfireCook(building: GameBuilding, cookable: CampfireCookable): (() => Promise<void>) | null {
      ...
      floatingText('-1 ' + cookable.inputName, 'loss');
      ...
      return async () => {
        if (cookable.output) {
          ...
          floatingText('+1 ' + cookable.outputName, 'gain');
        }
    ...
    async craftTorch() {
      ...
      floatingText('-1 树枝', 'loss');
      inventory.removeItem('grass', 2);
      floatingText('-2 干草', 'loss');
      ...
      return [
        ...
        },
        {
          ...
          preExecute: () => {
            ...
            floatingText('-3 树枝', 'loss');
            inventory.removeItem('ore', 2);
            floatingText('-2 矿石', 'loss');
        ...
        },
        {
          ...
          preExecute: () => {
            ...
            floatingText('-2 树枝', 'loss');
            inventory.removeItem('ore', 3);
            floatingText('-3 矿石', 'loss');
            character.energy = Math.max(0, character.energy - 5);
            return true;
          },
          handler: async () => {
    ...
    repairTrap(building: import('./types').GameBuilding) {
      ...
      for (const [res, amount] of Object.entries(TRAP_REPAIR_COST)) {
        ...
        floatingText('-' + amount + ' ' + (RESOURCE_NAMES[res] ?? res), 'loss');
      }
    ...
    destroyTrap(building: import('./types').GameBuilding) {
      ...
      for (const [res, amount] of Object.entries(TRAP_DESTROY_RETURN)) {
        ...
        floatingText('+' + amount + ' ' + (RESOURCE_NAMES[res] ?? res), 'gain');
      }
    ...
    harvestFarm(building: import('./types').GameBuilding) {
      ...
      floatingText('+' + amount + ' ' + cropConfig.name, 'gain');
      // 重置农田状态
